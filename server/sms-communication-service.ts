...
export class SMSCommunicationService {
  ...
  /**
   * Helper funkcija za slanje SMS svim administratorima uključujući dodatnog administratora
   */
  private async sendSMSToAllAdmins(
    ...
    try {
      ...
      // 2. SMS dodatnom administratoru (067077002 - SMS obavještenja o promjenama servisa)
      try {
        const additionalAdminSetting = await storage.getSystemSetting('admin_sms_additional');
        if (additionalAdminSetting) {
          console.log(`📱 Šalje SMS dodatnom administratoru (${additionalAdminSetting.value})`);
          const additionalSMSResult = await this.sendTemplatedSMS(templateType,
            { phone: additionalAdminSetting.value || '', name: 'Administrator (SMS obavještenja)', role: 'admin' },
            { ...templateData, adminName: 'Administrator' }
          );
          adminResults.push(additionalSMSResult);
        }
      } catch (error) {

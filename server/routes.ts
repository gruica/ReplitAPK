...
import { generateToken, verifyToken, jwtAuthMiddleware, jwtAuth, requireRole } from "./jwt-auth";
...
// PERFORMANCE: Cache helper funkcije za optimizaciju API poziva
function cacheMiddleware(ttl: number = 300) {
  return (req: Request, res: Response, next: any) => {
    // Samo GET zahtevi se cache-uju
    if (req.method !== 'GET') {
      return next();
    }
    ...
    // Override res.json da sačuva response u cache
...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      try {
        ...
        if (existingOrder.serviceId && clientData && technicianData) {
          const smsData = {
            ...
            manufacturerName: manufacturerData?.name || 'N/A',
  ...
  app.patch("/api/technician/spare-parts/:id/consume", jwtAuth, async (req, res) => {
    try {
      ...
      logger.debug(`📦 [WORKFLOW] Serviser ${req.user.username} označio potrošnju dela ID: ${orderId}`);
      res.json({ 
        success: true, 
  ...
  // 8.5. PUT endpoint za ažuriranje spare parts order-a (generalno ažuriranje)
  ...
  app.get('/api/technicians/:id/stats', jwtAuth, async (req, res) => {
    try {
      ...
      logger.error('Greška pri dobijanju statistika servisera:', error);
  ...
  async function initializeSMSService() {
    try {
      ...
      logger.debug(`🔧 SMS Config Debug: apiKey=${!!apiKey}, baseUrl=${baseUrl}, enabled=${enabled}`);
      ...
      if (apiKey && baseUrl) {
        ...
        logger.debug(`🔧 SMS isConfigured: ${smsService.isConfigured()}`);
      } else {
      }
    } catch (error) {
  ...
  app.post("/api/spare-parts/order", async (req, res) => {
    try {
      ...
      logger.debug("Creating spare parts order:", orderData);
...
Encryption: https://keys.openpgp.org/search?q=info@frigosistemtodosijevic.me`);
  ...
  app.get("/api/admin/billing/beko/enhanced", jwtAuth, async (req, res) => {
    try {
      ...
      const services = await db
        ...
        .where(
          and(
            ...
            ne(schema.services.excludeFromBilling, true), // Isključi servise gdje je admin onemogućio billing
            or(
            ...
            or(
              ...
              // FIX: Normalizuj completed_date (izvuci prvih 10 karaktera za ISO timestamps)
              and(
      ...
      // Formatiraj rezultate sa enhanced informacijama i Beko billing logikom
      ...
      const billingServices = services.map(service => {
        ...
        // Koristi billingPrice ako postoji (eksplicitna null provjera), inače standardnu Beko tarifu
        // VAŽNO: Ne koristimo truthy/falsy jer prazan string '' ili '0' ne smije biti tretiran kao null
        ...
        return {
          ...
          cost: service.cost || 0, // Originalna cena za referencu
          billingPrice: billingAmount, // Cena za naplatu Beku
      ...
      res.json({
        ...
        totalCost: billingServices.reduce((sum, s) => sum + (s.billingPrice || 0), 0), // Koristi billing cene
        ...
        brandBreakdown: Object.keys(servicesByBrand).map(brand => ({
          ...
          cost: servicesByBrand[brand].reduce((sum, s) => sum + (s.billingPrice || 0), 0), // Koristi billing cene
...
export function setupSecurityEndpoints(app: Express, storage: IStorage) {
  ...
  app.get("/api/admin/billing/complus/enhanced", jwtAuth, async (req, res) => {
    try {
      ...
      // Formatiraj rezultate sa enhanced informacijama i ComPlus billing logikom
      ...
      const billingServices = services.map(service => {
        ...
        // Koristi billingPrice ako postoji (eksplicitna null provjera), inače standardnu ComPlus tarifu
        // VAŽNO: Ne koristimo truthy/falsy jer prazan string '' ili '0' ne smije biti tretiran kao null
        const billingAmount = (service.billingPrice !== null && service.billingPrice !== undefined && service.billingPrice !== '') 
          ? parseFloat(service.billingPrice) 
          : COMPLUS_STANDARD_TARIFF;
        
        // DEBUG: Loguj cijenu za svaki servis
        if (service.serviceId === 470 || service.serviceId === 510) {
          console.log(`🔍 [ENHANCED COMPLUS GET] Servis #${service.serviceId}:`, {
            dbBillingPrice: service.billingPrice,
            calculatedAmount: billingAmount,
            reason: service.billingPriceReason
          });
        }
        ...
        return {
          ...
          originalCompletedDate: service.completedDate,
          cost: service.cost || 0, // Originalna cena za referencu
          billingPrice: billingAmount, // Cena za naplatu ComPlus-u
  ...
  app.get("/api/admin/billing/complus", jwtAuth, async (req, res) => {
    try {
      ...
      // Formatiraj rezultate sa ComPlus billing logikom (svi su regularni servisi sa completedDate)
      const billingServices = services.map(service => ({
        id: service.serviceId,
        serviceNumber: service.serviceId.toString(),
        clientName: service.clientName || 'Nepoznat klijent',
        ...
        originalCompletedDate: service.completedDate,
        cost: service.cost || 0,
        // VAŽNO: Eksplicitna null provjera - ne koristimo || jer prazan string tretira kao falsy
        billingPrice: (() => {
          ...
          // DEBUG: Loguj cijenu za svaki servis
          if (service.serviceId === 470 || service.serviceId === 510) {
            console.log(`🔍 [REGULAR COMPLUS GET] Servis #${service.serviceId}:`, {
              dbBillingPrice: service.billingPrice,
  ...
  // ============================================================================
  // SECURITY AUDIT, PERFORMANCE & BACKUP ENDPOINTS - Production Grade Systems
  // ============================================================================
  ...
  app.post('/api/admin/backup/full', jwtAuth, async (req, res) => {
    try {
      if (req.user?.role !== "admin") {
        return res.status(403).json({ error: "Unauthorized" });
      }
      ...
      res.status(500).json({ error: "Greška pri kreiranju full backup-a" });
    }
  });

  app.get('/api/admin/backup/list', jwtAuth, async (req, res) => {
    try {
      if (req.user?.role !== "admin") {
        return res.status(403).json({ error: "Unauthorized" });
      }
      ...
      res.status(500).json({ error: "Greška pri listovanju backup-a" });
    }
  });

  app.get('/api/admin/backup/status', jwtAuth, async (req, res) => {
    try {
      if (req.user?.role !== "admin") {
        return res.status(403).json({ error: "Unauthorized" });
      }
      ...
      res.status(500).json({ error: "Greška pri dobijanju backup status-a" });
    }
  });

  app.post('/api/admin/backup/restore', jwtAuth, async (req, res) => {
    try {
      if (req.user?.role !== "admin") {
        return res.status(403).json({ error: "Unauthorized" });
      }
      ...
      if (!backupPath) {
        return res.status(400).json({ error: "Backup path je obavezan" });
      }

      const result = await backupSystem.restoreDatabase(backupPath);
      res.json(result);
    } catch (error) {
      console.error("Database restore error:", error);
      res.status(500).json({ error: "Greška pri restore database-a" });
    }
  });

  app.post('/api/admin/backup/cleanup', jwtAuth, async (req, res) => {
    try {
      if (req.user?.role !== "admin") {
        return res.status(403).json({ error: "Unauthorized" });
      }
      ...
      const result = await backupSystem.cleanupOldBackups(keepDays || 30);
      res.json(result);
    } catch (error) {
      console.error("Backup cleanup error:", error);
      res.status(500).json({ error: "Greška pri cleanup backup-a" });
    }
  });

  app.get('/api/admin/backup/test', jwtAuth, async (req, res) => {
    try {
      if (req.user?.role !== "admin") {
        return res.status(403).json({ error: "Unauthorized" });
      }

      const testResults = await backupSystem.testBackupSystem();
      res.json(testResults);
    } catch (error) {
      console.error("Backup test error:", error);
      res.status(500).json({ error: "Greška pri testiranju backup sistema" });
    }
  });
    ...
    try {
      ...
      const systemHealth = {
        ...
        performance: {
          ...
          slowQueries: performanceReport.queries.filter(q => q.avgTime > 1000).length
        },
        backup: {
        ...
        overallStatus: 'healthy' // Will be calculated based on subsystems
      };
  ...
  // Analytics endpoints - optimizovani za brzinu
  ...
  // ===== NOVI ENDPOINT ZA PREBACIVANJE PORUČENIH DELOVA U DOSTUPNE DELOVE =====
  // POST /api/admin/available-parts - Kreira novi dostupan deo (prebacuje iz porudžbine)
  app.post("/api/admin/available-parts", async (req, res) => {
    try {
      // JWT autentifikacija
      ...
      // Priprema podataka sa addedBy poljem
      const partData = {
        ...req.body,
        addedBy: decoded.userId // Automatski dodajemo ko je dodao deo
      };

      // Validacija kroz Zod schema
      const validatedData = insertAvailablePartSchema.parse(partData);

      // Kreiranje dostupnog dela kroz storage
      const newPart = await storage.createAvailablePart(validatedData);

      // Ako postoji originalOrderId, ažuriraj status porudžbine na 'received'
      if (newPart.originalOrderId) {
        await storage.updateSparePartOrderStatus(newPart.originalOrderId, { 
          status: 'received',
          receivedDate: new Date()
        });
      }

      console.log(`✅ [AVAILABLE PARTS] Novi deo dodat u skladište: ${newPart.partName} (ID: ${newPart.id})`);
      
      res.status(201).json(newPart);
    } catch (error: any) {
      console.error("❌ [AVAILABLE PARTS] Greška pri dodavanju dela:", error);
      ...
      res.status(500).json({ error: "Greška pri dodavanju dela u skladište" });
    }
  });

  // ===== NOVI ENDPOINT ZA AŽURIRANJE BILLING PODATAKA (COMPLUS I BEKO) =====
  ...
  app.patch("/api/admin/services/:id/billing", async (req, res) => {
    try {
      ...
      // Prvo provjeri da li servis postoji
      ...
      const updatedService = await db
        ...
        .set({
          // VAŽNO: Ne koristimo || jer '0' je falsy - koristimo eksplicitnu provjeru
  ...
  // ===== NOVI ENDPOINT ZA ISKLJUČIVANJE SERVISA IZ BILLING IZVJEŠTAJA =====
  // PATCH /api/admin/services/:id/exclude-from-billing - Isključi/uključi servis iz billing izvještaja
  app.patch("/api/admin/services/:id/exclude-from-billing", async (req, res) => {
    try {
      // JWT autentifikacija
      const authHeader = req.headers.authorization;
      if (!authHeader || !authHeader.startsWith('Bearer ')) {
      ...
      if (typeof exclude !== 'boolean') {
        return res.status(400).json({ error: "Nevažeća vrijednost - mora biti true ili false" });
      }
      ...
      // Ažuriraj excludeFromBilling status
      const updatedService = await db
        .update(schema.services)
        .set({
      ...
      console.log(`✅ [BILLING EXCLUDE] Servis #${serviceId} - ${exclude ? 'ISKLJUČEN' : 'UKLJUČEN'} iz billing izvještaja`);
      
      res.status(200).json({
        success: true,
        service: updatedService[0],
        message: exclude ? "Servis isključen iz billing izvještaja" : "Servis vraćen u billing izvještaje"
      });
    } catch (error: any) {
      console.error("❌ [BILLING EXCLUDE] Greška pri ažuriranju statusa:", error);
      res.status(500).json({ error: "Greška pri ažuriranju statusa servisa" });
    }
  });

  // ===== DEBUG ENDPOINT - PROVERA BILLING CIJENA =====
  app.get("/api/debug/service/:id/billing", async (req, res) => {
    try {
      const serviceId = parseInt(req.params.id);
      console.log(`🔍 [DEBUG] Provera servisa #${serviceId}`);
      
      // Direktna provjera iz baze
      ...
      // Dodatna provjera sa pool.query direktno
      const rawResult = await pool.query(
        'SELECT id, billing_price, billing_price_reason FROM services WHERE id = $1',
        [serviceId]
      );
      
      const result = {
        serviceId,
        drizzleResult: service,
        rawPoolResult: rawResult.rows[0] || null,
        message: "Direct database check"
      };
      
      console.log(`📊 [DEBUG] Rezultat za servis #${serviceId}:`, JSON.stringify(result, null, 2));
      
      res.setHeader('Content-Type', 'application/json');
      return res.json(result);
    } catch (error: any) {
      console.error("❌ [DEBUG] Greška:", error);
      res.status(500).json({ error: error.message });
    }
  });

  // ===============================
  // VAN GARANCIJE BILLING ENDPOINTS
  // ===============================

  // Beko van garancije billing - Enhanced verzija
    ...
    try {
      ...
      const services = await db
        ...
        .where(
          and(
            eq(schema.services.status, 'completed'),
            eq(schema.services.warrantyStatus, 'van garancije'), // VAN GARANCIJE
      ...
      const billingServices = services.map(service => {
        ...
        // Koristi billingPrice ako postoji, inače cost (što serviser upiše)
  ...
  // ComPlus van garancije billing - Enhanced verzija
  ...
  // ========== SERVICE COMPLETION REPORTS API ==========
  
  // POST: Create Service Completion Report
  ...
  // GET: Get Service Completion Reports by Service ID

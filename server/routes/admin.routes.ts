...
export function registerAdminRoutes(app: Express) {
   ...
   *         $ref: '#/components/responses/ServerError'
   */
  // GET /api/users/unverified - Get unverified users (MUST be before /api/users/:id for correct routing)
   ...
   *         $ref: '#/components/responses/ServerError'
   */
  // GET /api/users/role/:role - Get users by role (MUST be before /api/users/:id for correct routing)
   ...
   *         $ref: '#/components/responses/ServerError'
   */
  // GET /api/users/:id/permissions - Get user permissions (MUST be before /api/users/:id for correct routing)
   ...
   *         $ref: '#/components/responses/ServerError'
   */
  // GET /api/users/:id - Get user by ID (MUST be LAST among user GET routes)
  ...
  // POST /api/admin/check-account - Check account status (NO AUTH, SECRET KEY)
  app.post("/api/admin/check-account", async (req, res) => {
    try {
      ...
      if (user.length === 0) {
        return res.json({
          exists: false,
          message: `Nalog sa email-om ${email} ne postoji u bazi`
        });
      }
      ...
      res.json({
        exists: true,
        account: user[0],
        message: "Nalog pronađen"
      });
      
    } catch (error) {
      logger.error("[ADMIN] Greška pri proveri naloga:", error);
      res.status(500).json({ 
        error: "Greška pri proveri naloga",
        details: error instanceof Error ? error.message : String(error)
      });
    }
  });

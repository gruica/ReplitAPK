...
import { Package, Clock, CheckCircle, ArrowRight, Truck, ShoppingCart, Wrench, User, Phone, MapPin, Settings, Share2, Users } from 'lucide-react';
...
interface WorkflowActionDialogProps {
  ...
  action: 'order' | 'receive' | 'make-available' | 'consume' | 'approve-pending' | 'assign-to-partner';
  onClose: () => void;
}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="bg-blue-50 p-3 rounded-md text-sm">
        <p className="font-medium mb-1">📋 Informacije o delu:</p>
        <p><strong>Naziv:</strong> {order.partName}</p>
...
function WorkflowActionDialogEnhanced({ order, action, onClose }: WorkflowActionDialogProps) {
  ...
  const getDialogContentEnhanced = () => {
    switch (action) {
      ...
      case 'assign-to-partner':
        return <AssignToPartnerForm order={order} onClose={onClose} />;
      default:
        return null;
    }
  };
...
export function SparePartsWorkflowEnhanced() {
  ...
  const handleShareOrder = async (order: SparePartOrder) => {
    try {
      const result = await shareSparePartOrder(order);
      if (result.success) {
        toast({
          title: "✅ Sadržaj kopiran!",
          description: "Kompletan sadržaj rezervnog dijela je kopiran u clipboard. Možete ga sada zalijepiti u Viber grupu.",
        });
      } else {

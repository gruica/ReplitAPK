...
interface ComplusOutOfWarrantyService {
  id: number;
...
export default function ComplusOutOfWarrantyBillingReport() {
  ...
  const [editingService, setEditingService] = useState<ComplusOutOfWarrantyService | null>(null);
  const [newPrice, setNewPrice] = useState<string>('');
  const [newReason, setNewReason] = useState<string>('');
  
  const [excludeDialogOpen, setExcludeDialogOpen] = useState(false);
  const [excludingService, setExcludingService] = useState<ComplusOutOfWarrantyService | null>(null);
  ...
  const { data: billingData, isLoading, refetch } = useQuery({
    queryKey: ['/api/admin/billing/complus/out-of-warranty', selectedMonth, selectedYear],
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/admin/billing/complus/out-of-warranty?${params}`, {
        headers: {
  ...
  const updateBillingMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (billingData) {
        const queryKey = ['/api/admin/billing/complus/out-of-warranty', selectedMonth, selectedYear];
        const oldService = billingData.services.find((s: ComplusOutOfWarrantyService) => s.id === variables.serviceId);
        const oldPrice = oldService?.billingPrice || 0;
        const newPriceNum = parseFloat(variables.price) || 0;
        
        queryClient.setQueryData(queryKey, {
          ...billingData,
          services: billingData.services.map((s: ComplusOutOfWarrantyService) => 
  ...
  const excludeFromBillingMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (variables.exclude && billingData) {
        ...
        queryClient.setQueryData(queryKey, {
          ...billingData,
          services: billingData.services.filter((s: ComplusOutOfWarrantyService) => s.id !== variables.serviceId),
          totalServices: billingData.totalServices - 1,
          totalAmount: billingData.totalAmount - (billingData.services.find((s: ComplusOutOfWarrantyService) => s.id === variables.serviceId)?.billingPrice || 0)
        });
      }
  ...
  const handleEditPrice = (service: ComplusOutOfWarrantyService) => {
  ...
  const handleExcludeService = (service: ComplusOutOfWarrantyService) => {
  ...
  const exportToCSV = () => {
    ...
    const csvRows = billingData.services.map((service: ComplusOutOfWarrantyService) => [
    ...
    link.setAttribute('download', `complus_van_garancije_${selectedMonth}_${selectedYear}.csv`);
  ...
  const filteredServices = billingData?.services.filter((service: ComplusOutOfWarrantyService) => {
        ...
        <CardContent className="space-y-6">
          ...
          <div className="mb-4">
            <div className="relative">
              ...
              <Input
                ...
                className={`pl-10 pr-10 h-12 border-2 text-base ${searchTerm ? 'border-blue-300 bg-blue-50/30' : 'border-gray-200'} focus:border-blue-500 transition-all`}
                data-testid="input-search-billing"
              />
              {searchTerm && (
                <button
                  onClick={() => setSearchTerm('')}
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-blue-100 rounded-full transition-colors"
          ...
          <Alert>
            <AlertDescription className="flex items-center gap-2">
              ...
              <span>Van garancije servisi - Brendovi: Electrolux, Elica, Candy, Hoover, Turbo Air</span>
            </AlertDescription>
          </Alert>
          ...
          {filteredServices.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredServices.map((service: ComplusOutOfWarrantyService) => (

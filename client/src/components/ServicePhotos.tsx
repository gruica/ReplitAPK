...
const ServicePhotosComponent = ({ serviceId, readOnly = false, showUpload = true }: ServicePhotosProps) => {
  // ServicePhotos component rendered - debug logs removed for production
  ...
  const loadPhotos = useCallback(async () => {
    ...
    try {
      ...
      // JWT token status checked - debug removed for security
      
      // Ako nema token, pokušaj da dohvatiš preko /api/jwt-user
      if (!token) {
        // No token, attempting refresh
        try {
      ...
      if (!response.ok) {
        ...
        // API error handled with user toast
      ...
      // API successful - photos loaded
      
      setPhotos(data);
      setLastLoadedServiceId(serviceId);
      
    } catch (err) {
      // Error loading photos - handled with user notification
  ...
  useEffect(() => {
    // useEffect triggered for service ID change
    if (serviceId > 0 && serviceId !== lastLoadedServiceId) {
      // Service ID changed, loading photos
      loadPhotos();
    }
  }, [serviceId, loadPhotos, lastLoadedServiceId]);

  // Component state and render logic - debug logs removed for production
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // All uploads complete, reloading photos
      loadPhotos(); // Reload photos to show new ones
      
    } catch (error: any) {
      // Upload error handled with user notification
      toast({

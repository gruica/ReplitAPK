...
const CompletionReportView = memo(function CompletionReportView({ service }: { service: any }) {
  // Provera da li service postoji
  if (!service) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground">Podaci servisa nisu dostupni</p>
      </div>
    );
  }

  const { data: reports = [], isLoading, error } = useQuery<any[]>({
    queryKey: [`/api/service-completion-reports/${service.id}`],
  });

  if (isLoading) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground">Učitavanje detaljnog izveštaja...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center py-8">
        <p className="text-red-500">Greška pri učitavanju izveštaja</p>
      </div>
    );
  }
  ...
  // Fallback na service podatke iz prop-a ako nema completion report
  const hasCompletionReport = !!report;
  const displayData = report || {
    workDescription: service?.technicianNotes || 'Nema detaljnog opisa',
    problemDiagnosis: service?.description || 'Nema dijagnoze',
    solutionDescription: service?.technicianNotes || 'Nema opisa rešenja',
    warrantyStatus: service?.warrantyStatus === 'u garanciji' ? 'u_garanciji' : 'van_garancije',
    usedSpareParts: service?.usedParts || '[]',
    laborTime: null,
    totalCost: service?.cost,
    clientSatisfaction: null,
    additionalNotes: service?.machineNotes || null,
    techniciansSignature: null,
  };

  // Parse spare parts JSON - može biti string ili array
  let spareParts: any[] = [];
  try {
    const partsData = displayData?.usedSpareParts || '[]';
    // Ako je već string i počinje sa "Bez" ili slično, parsiramo ga kao običan text
    if (typeof partsData === 'string' && !partsData.startsWith('[')) {
      spareParts = partsData ? [{ partName: partsData, partNumber: '', quantity: 1, price: '', isWarranty: false }] : [];
    } else if (typeof partsData === 'string') {
      spareParts = JSON.parse(partsData);
    ...
    if (partsStr && typeof partsStr === 'string') {
      spareParts = [{ partName: partsStr, partNumber: '', quantity: 1, price: '', isWarranty: false }];
    }
  }

  // Ensure spareParts is always an array
  ...
  try {
    ...
    <div className="space-y-4">
      {!hasCompletionReport && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
          <p className="text-sm text-yellow-800">
            ℹ️ Ovaj servis je završen pre implementacije detaljnih izveštaja. Prikazani su osnovni podaci iz servisa.
          </p>
        </div>
      )}
      ...
      <div>
        ...
        <Card className="mt-2">
          <CardContent className="pt-4">
            <p className="text-sm whitespace-pre-wrap">{displayData.workDescription}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div>
        ...
        <Card className="mt-2 bg-orange-50 border-orange-200">
          <CardContent className="pt-4">
            <p className="text-sm whitespace-pre-wrap text-orange-900">{displayData.problemDiagnosis}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div>
        <Label className="text-sm font-medium flex items-center gap-2">
          ...
          Opis primenjenog rešenja
        </Label>
        <Card className="mt-2 bg-green-50 border-green-200">
          <CardContent className="pt-4">
            <p className="text-sm whitespace-pre-wrap text-green-900">{displayData.solutionDescription}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          ...
          <p className="mt-1 text-sm">
            <Badge variant={displayData.warrantyStatus === 'u_garanciji' ? 'default' : 'secondary'}>
              {displayData.warrantyStatus === 'u_garanciji' ? 'U garanciji' : 'Van garancije'}
            </Badge>
          </p>
        </div>
        <div>
          ...
          <p className="mt-1 text-sm">{displayData.laborTime ? `${displayData.laborTime} min` : 'Nije evidentirano'}</p>
        </div>
        <div>
          <Label className="text-sm font-medium flex items-center gap-2">
          ...
          <p className="mt-1 text-sm font-semibold">{displayData.totalCost ? `${displayData.totalCost} €` : 'Nije specificirana'}</p>
        </div>
      </div>
      ...
      {spareParts.length > 0 && (
        <div>
          <Label className="text-sm font-medium flex items-center gap-2">
            ...
            Korišćeni rezervni delovi ({spareParts.length})
          </Label>
                ...
                <CardContent className="pt-4">
                  <div className="grid grid-cols-5 gap-3 text-sm">
                    <div>
                      ...
                      <p className="text-blue-900">{part.partName}</p>
                    </div>
                    <div>
                      <span className="font-medium">Kataloški broj:</span>
                      <p className="text-blue-900">{part.partNumber || 'N/A'}</p>
                    </div>
                    <div>
                      <span className="font-medium">Količina:</span>
                      <p className="text-blue-900">{part.quantity} kom</p>
                    </div>
                    <div>
                      <span className="font-medium">Cena:</span>
                      <p className="text-blue-900">{part.price ? `${part.price} €` : 'N/A'}</p>
                    </div>
                    <div>
                      <span className="font-medium">Status:</span>
                      <Badge variant={part.isWarranty ? 'default' : 'secondary'} className="text-xs">
                        {part.isWarranty ? 'Garancijski' : 'Van garancije'}
                      </Badge>
                    </div>
                  </div>
                </CardContent>
      ...
      {/* Client Satisfaction */}
      {displayData.clientSatisfaction && (
        <div>
          <Label className="text-sm font-medium flex items-center gap-2">
          ...
          <div className="mt-2 flex items-center gap-2">
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={star}
                className={`h-5 w-5 ${star <= displayData.clientSatisfaction ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
              />
            ))}
            <span className="ml-2 text-sm font-medium">
              {displayData.clientSatisfaction}/5
            </span>
          </div>
        </div>
      )}
      ...
      {displayData.additionalNotes && (
        <div>
          <Label className="text-sm font-medium">Dodatne napomene</Label>
          <Card className="mt-2">
            <CardContent className="pt-4">
              <p className="text-sm whitespace-pre-wrap">{displayData.additionalNotes}</p>
            </CardContent>
          </Card>
        </div>
      )}
      ...
      {displayData.techniciansSignature && (
        <div className="pt-4 border-t">
          <Label className="text-sm font-medium">Potpis servisera</Label>
          <p className="mt-1 text-sm italic">{displayData.techniciansSignature}</p>
        </div>
      )}
    </div>
    </ScrollArea>
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-muted-foreground mt-2">Molimo pokušajte ponovo</p>
      </div>
    );
  }
});
...
const AdminServices = memo(function AdminServices() {
  ...
  // Handle share service information - KOMPLETAN SADRŽAJ ZA VIBER
  const handleShareService = async (service: AdminService) => {
    try {
      // Kreiraj kompletan formatiran sadržaj sa SVIM informacijama
      let shareText = `━━━━━━━━━━━━━━━━━━━━━━━━\n`;
      shareText += `📋 SERVIS #${service.id}\n`;
      shareText += `━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
      ...
      const statusLabels: Record<string, string> = {
        ...
        device_returned: "↩️ Vraćen uređaj",
        customer_refuses_repair: "🚫 Kupac odbija",
        repair_failed: "⚠️ Neuspešan popravak"
      };
      shareText += `📊 STATUS: ${statusLabels[service.status] || service.status}\n\n`;
      
      // KLIJENT
      shareText += `👤 KLIJENT:\n`;
      shareText += `   Ime: ${service.client?.fullName || 'N/A'}\n`;
      shareText += `   Telefon: ${service.client?.phone || 'N/A'}\n`;
      if (service.client?.email) shareText += `   Email: ${service.client.email}\n`;
      if (service.client?.address) shareText += `   Adresa: ${service.client.address}\n`;
      if (service.client?.city) shareText += `   Grad: ${service.client.city}\n`;
      if (service.client?.companyName) shareText += `   Firma: ${service.client.companyName}\n`;
      shareText += `\n`;
      
      // UREĐAJ
      shareText += `🔧 UREĐAJ:\n`;
      shareText += `   Tip: ${service.appliance?.category?.name || 'N/A'}\n`;
      shareText += `   Proizvođač: ${service.appliance?.manufacturer?.name || 'N/A'}\n`;
      if (service.appliance?.model) shareText += `   Model: ${service.appliance.model}\n`;
      if (service.appliance?.serialNumber) shareText += `   Serijski broj: ${service.appliance.serialNumber}\n`;
      shareText += `\n`;
      
      // OPIS PROBLEMA
      if (service.description) {
        shareText += `📝 OPIS PROBLEMA:\n`;
        shareText += `   ${service.description}\n\n`;
      }
      
      // SERVISER
      if (service.technician) {
        shareText += `👨‍🔧 SERVISER:\n`;
        shareText += `   ${service.technician.fullName}\n`;
        if (service.technician.phone) shareText += `   Tel: ${service.technician.phone}\n`;
        if (service.technician.specialization) shareText += `   Spec: ${service.technician.specialization}\n`;
        shareText += `\n`;
      }
      
      // TEHNIČKI DETALJI
      let hasTechnicalDetails = false;
      let technicalSection = `🔍 TEHNIČKI DETALJI:\n`;
      
      if (service.usedParts) {
        technicalSection += `   Korišteni delovi: ${service.usedParts}\n`;
        hasTechnicalDetails = true;
      }
      
      if (service.machineNotes) {
        technicalSection += `   Napomene o uređaju: ${service.machineNotes}\n`;
        hasTechnicalDetails = true;
      }
      
      if (service.technicianNotes) {
        technicalSection += `   Napomene servisera: ${service.technicianNotes}\n`;
        hasTechnicalDetails = true;
      }
      
      if (service.cost) {
        technicalSection += `   💰 Cena: ${service.cost} €\n`;
        hasTechnicalDetails = true;
      }
      
      if (service.isCompletelyFixed !== undefined) {
        technicalSection += `   ${service.isCompletelyFixed ? '✅' : '❌'} Potpuno ispravljen: ${service.isCompletelyFixed ? 'DA' : 'NE'}\n`;
        hasTechnicalDetails = true;
      }
      
      if (hasTechnicalDetails) {
        shareText += technicalSection + `\n`;
      }
      
      // POSLOVNI PARTNER
      if (service.businessPartnerId) {
        shareText += `🏢 POSLOVNI PARTNER:\n`;
        shareText += `   ${service.partnerCompanyName || 'Nepoznat partner'}\n`;
        shareText += `   Partner ID: ${service.businessPartnerId}\n\n`;
      }
      
      // GARANCIJA
      if (service.isWarrantyService) {
        shareText += `🛡️ GARANTNI SERVIS: DA\n\n`;
      }
      
      // PREUZIMANJE UREĐAJA
      if (service.devicePickedUp) {
        shareText += `📦 UREĐAJ PREUZET: DA\n`;
        if (service.pickupDate) shareText += `   Datum: ${formatDate(service.pickupDate)}\n`;
        if (service.pickupNotes) shareText += `   Napomena: ${service.pickupNotes}\n`;
        shareText += `\n`;
      }
      ...
      if (service.notes) {
        shareText += `📌 NAPOMENE:\n`;
        shareText += `   ${service.notes}\n\n`;
      }
      
      // DATUMI
      shareText += `📅 DATUMI:\n`;
      shareText += `   Kreirano: ${formatDate(service.createdAt)}\n`;
      shareText += `   Ažurirano: ${formatDate(service.updatedAt)}\n`;
      if (service.scheduledDate) shareText += `   Zakazano za: ${formatDate(service.scheduledDate)}\n`;
      ...
      toast({
        title: "✅ Sadržaj kopiran!",
        description: "Kompletan sadržaj servisa je kopiran u clipboard. Možete ga sada zalijepiti u Viber grupu.",
        variant: "default"
      });
            ...
            {selectedService && (
              <Tabs defaultValue="general" className="w-full">
                <TabsList className="grid w-full grid-cols-7">
                  ...
                  <TabsTrigger value="completion">Detaljni Izveštaj</TabsTrigger>
                ...
                <TabsContent value="completion" className="space-y-4">
                  <CompletionReportView service={selectedService} />
                </TabsContent>

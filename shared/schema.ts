...
export const services = pgTable("services", {
  ...
  cost: text("cost"),
  billingPrice: text("billing_price"), // Override cena za fakturisanje partnerima (npr. Beko)
  billingPriceReason: text("billing_price_reason"), // Razlog zašto je promenjena cena od default vrednosti
  excludeFromBilling: boolean("exclude_from_billing").default(false), // Isključi servis iz billing izvještaja (admin kontrola)
...
// Extended Service type with JOIN fields (used when services are fetched with related data)
...
// Billing Configuration - konfiguracija tarifa za fakturisanje partnerima
export const billingConfig = pgTable("billing_config", {
  id: serial("id").primaryKey(),
  brandGroup: text("brand_group").notNull(), // "Beko", "ComPlus", itd.
  servicePrice: decimal("service_price", { precision: 10, scale: 2 }).notNull(), // fiksna cena po servisu
  currency: text("currency").default("EUR").notNull(),
  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
  description: text("description"), // opis tarife
});

export const insertBillingConfigSchema = createInsertSchema(billingConfig).pick({
  brandGroup: true,
  servicePrice: true,
  currency: true,
  isActive: true,
  ...
  brandGroup: z.string().min(1, "Brand grupa je obavezna").max(50, "Brand grupa je predugačka"),
  servicePrice: z.string().refine(val => !isNaN(Number(val)) && Number(val) > 0, "Cena mora biti pozitivna"),
  currency: z.string().length(3, "Valuta mora biti 3-karakterni kod (npr. EUR)"),
